...
[properties])= (the modify part is optional). The following will create a
context called "leasure" in which only items that are tagged with "fun" as well
...
#+BEGIN_EXAMPLE
> context add leasure #fun, (+active due<=now urgency>=10) default review:tomorrow
#+END_EXAMPLE
